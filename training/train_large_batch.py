...
def model_process(curr_batch, model, tokenizer, model_name):
    ...
    if model_name in ["e5", "simcse", "roberta", "specterv2", "simlm", "spladev2", "scibert", "ernie"]:
        batch_dict = tokenizer(curr_batch, padding='max_length', max_length=512, truncation=True,
                                   return_tensors='pt')
...
# Integrated code:
def training_model_large_batch(batch_size, num_epochs, name, cuda, margin, shuffle, half, model_name, max_breach =5):
    ...
    # with open(f"data_prep/train_test_data/test_{name}.pickle", "rb") as f:
    #     test_data = pickle.load(f)

    print(f"length of train data triplets {len(train_data)}")
    # print(f"length of test data triplets {len(test_data)}")
    
    assert batch_size >1  # we want to set batch_size to 80
    
    train_dataset = MyDataset(train_data)

    train_dataloader = DataLoader(train_dataset, batch_size=batch_size,
                                  shuffle=shuffle, drop_last = True)  #### NOTE HERE PLEASE< SHUFFLE !!!!!!
    ...
    accumulation = batch_size  # used to be 80
    ...
    scheduler = Scheduler(optimizer=optimizer,
                          T_0=T_0)  # dataset size / mini_batch_size / delay (mini_batch_size * delay = 60-100)

    delay = accumulation // batch_size  # change with mini_batch_size
    ...
    for epoch in range(num_epochs):
        ...
        torch.load(f'./training/{model_path}/{name}_model_{model_name}_{epoch + 1}.pth', map_location='cpu')
